...
**Пример использование await для последовательных операций**
```python
import asyncio
...
if __name__ == '__main__':
    asyncio.run(main())
"""
Начинаем чтение из файла exemple.txt 
Чтение из файла exemple.txt завершено
...
# Even loop цикл событий
Цикл событий, или event loop, это сердце любого асинхронного кода. Его можно представить как обычный цикл, который работает в одном потоке и занимается организацией задач: он отмечает, когда какая задача должна начаться, и запускает её в нужный момент.
...
asyncio.run(main())
"""
 Пиццерия Тесто и сыр начала готовить пиццу для заказа 1
 Пиццерия Тесто и сыр начала готовить пиццу для заказа 2
 Пиццерия Тесто и сыр начала готовить пиццу для заказа 3
 Пиццерия Тесто и сыр начала готовить пиццу для заказа 4
 Пиццерия Тесто и сыр начала готовить пиццу для заказа 5
 Пиццерия Тесто и сыр закончила готовить пиццу для заказа 1
 Пиццерия Тесто и сыр закончила готовить пиццу для заказа 2
 Пиццерия Тесто и сыр закончила готовить пиццу для заказа 3
 Пиццерия Тесто и сыр закончила готовить пиццу для заказа 5
 Пиццерия Тесто и сыр закончила готовить пиццу для заказа 4
